...
% ~~~~~~~~~~~~~~~~~
 ...
 % Decide whether to simulate model output or use previous results if simulate is equal to  then the model is resimulated
...
if simulate ==1
    stochastic = 1;
    Wendling8_Sim_v1; % Simulate states and output of the extended neural mass model
...
% Dk =1; %Number of stochastic inputs in the system that need to be
% estimated
...
simulation_name = strcat(Estimation_type,'\',simulation_initial_name,'_Sim_',int2str(count),'_P_',int2str(Dp+Dk),'PE_',int2str(Percentage*100),'_N_',int2str(Measurement_noise*1e3),'mV_');

Error_name = strcat(Estimation_type,'\','Error_',simulation_initial_name,'_Sim_',int2str(count),'_P_',int2str(Dp+Dk),'PE_',int2str(Percentage*100),'_N_',int2str(Measurement_noise*1e3),'mV');

...
if (Image_handling_states(2,5) ==1) 
    ...
    axis([tstart zoomend (min(minc)-abs(min(minc))*scale) (max(maxc)+abs(max(maxc))*scale)]);
    k=legend('Simulated Zp','Estimated Zp','Location',legLoc);
    legend(k,'boxoff');
    set(k,'fontsize',legend_fontsize);
end
...
axis([0 max(t) (min(minc)-abs(min(minc))*scale) (max(maxc)+abs(max(maxc))*scale)]);
k=legend('Simulated Ze','Estimated Ze','Error on Estimate','Location',legLoc);
legend(k,'boxoff');
set(k,'fontsize',legend_fontsize);
% Print image with resolution 600 to pdf test.
...
if zoom>0
    ...
    axis([tstart zoomend (min(minc)-abs(min(minc))*scale) (max(maxc)+abs(max(maxc))*scale)]);
    k=legend('Simulated Ze','Estimated Ze','Location',legLoc);
    legend(k,'boxoff');
    set(k,'fontsize',legend_fontsize);
...
k=legend('Simulated Zsi','Estimated Zsi','Error on Estimate','Location',legLoc);
legend(k,'boxoff');
set(k,'fontsize',legend_fontsize);
...
if zoom>0
    ...
    ylabel('Rate of Change (mV/s)','fontsize',label_fontsize)
    %     title('Rate of Change of Vsi','fontsize', label_fontsize)
    k=legend('Simulated Zsi','Estimated Zsi','Location',legLoc);
    legend(k,'boxoff');
    set(k,'fontsize',legend_fontsize);
...
k=legend('Simulated Zfi','Estimated Zfi','Error on Estimate','Location',legLoc);
legend(k,'boxoff');
set(k,'fontsize',legend_fontsize);
...
if zoom>0
    ...
    ylabel('Rate of Change (mV/s)','fontsize',label_fontsize)
    %     title('Rate of Change of Vfi','fontsize', label_fontsize)
    k=legend('Simulated Zfi','Estimated Zfi','Location',legLoc);
    legend(k,'boxoff');
    set(k,'fontsize',legend_fontsize);
...
if (Image_handling_multi(1) ==1) 
% Plot all aggregate membrane potentials on a single plot
    multiplier = Ds/4;
    namesS = {'Vpo (mV)','Veo (mV)','Vsio (mV)','Vfio (mV)'};
    ...
    for k =1:Ds/2    
        ...
        %     title('Rate of Change of Vfi','fontsize', label_fontsize)
        m=legend('Simulated','Estimated','Location',legLoc);
        legend(m,'boxoff');
...
if Dk ==1
    if if (Image_handling_states(1,9) ==1) 
    ...
    k=legend('Simulated Input','Estimated Input','Error on Estimate','Location',legLoc);
    legend(k,'boxoff');
    set(k,'fontsize',legend_fontsize);
        end
    if (Image_handling_states(2,9) ==1) 
        ...
        ylabel('Firing rate (Hz)','fontsize',label_fontsize)
        %     title('Rate of Change of Vfi','fontsize', label_fontsize)
        k=legend('Simulated Input','Estimated Input','Location',legLoc);
        legend(k,'boxoff');
        set(k,'fontsize',legend_fontsize);
...
if Dp >0
    ...
    k= legend('Simulated','Estimated','Error on Estimate','Location',legLoc);
    legend(k,'boxoff');
    set(k,'fontsize',legend_fontsize);
       ...
       if (Image_handling_states(2,10) ==1) 
        ...
        ylabel('A (mV)','fontsize',label_fontsize)
        %     title('Rate of Change of Vfi','fontsize', label_fontsize)
        k=legend('Simulated','Estimated','Location',legLoc);
        legend(k,'boxoff');
        set(k,'fontsize',legend_fontsize);

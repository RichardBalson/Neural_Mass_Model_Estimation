...
% description
% ~~~~~~~~~~~
% this scriptassignes all static variables
...
if ((min(MVI(:,1))==max(MVI(:,1))) && (min(MVI(:,2))==max(MVI(:,2))) && (min(MVI(:,3))==max(MVI(:,3))))
    ...
    simulation_initial_name = ['Boon_UKF_WM8_f=',int2str(sampling_frequency),...
    ...
    m=1;
    simulation_initial_name = ['UKF_WM8_VP_f=',int2str(sampling_frequency),...
        '_A_',int2str(MVI(1,1)),'_B_',int2str(MVI(1,2)),'_G_',int2str(MVI(1,3)),'_DC_S_',int2str(stochasticv)]; % Initaite name for simulation, used for saving purposes
end
...
check = output8(EstStart*sampling_frequency+1:limit); % Assign a new variable as the simulated output

% Set Initial parameter values for the estimation procedure
% ~~~~~~~~~~~~~~~~~~~~

Initial_excitability = (1-Percentage)*MVI(1,1); % Intial excitability, estimate of parameter A
Initial_inhibition = (1-Percentage)*MVI(1,2); % Intial slow inhibition, estimate of parameter B
Initial_Finhibition = (1-Percentage)*MVI(1,3); % Intial fast inhibition, estimate of parameter G
...
if kappa > 0
    Number_sigma = 2*Dx+1;  % Specify number of sigma points to generate, if kappa is greater than zero than the mean is propagated
    % through the system and there is therefore one more sigma point generated
else
    Number_sigma = 2*Dx; % When kappa is set to zero sigma points are generated, note there are twice the number of sigma points as there are states
...
State_uncertainty(1,[2 6]) = ones(1,2)*(stochastic*Variable_state_uncertainty+Base_state_uncertainty);
 if m ==0
if Dp >0
    ...
    Parameter_std_deviation = [];
     ...
     if Dp >0
    Parameter_uncertainty(1,1) = Base_parameter_uncertainty + Variable_parameter_uncertainty_exc*m; % Variance to allow for model error and stochastic input effect
    Parameter_std_deviation(1,1) = max(Max_A-Initial_excitability,Initial_excitability-Min_A); % Variance to allow for tracking of parameters Parameters appear to work  5e-3
    if Dp >1
        Parameter_uncertainty(1,2) = (Base_parameter_uncertainty + Variable_parameter_uncertainty_inh*m);
        Parameter_std_deviation(1,2) = max(Max_B-Initial_inhibition,Initial_inhibition-Min_B);
        if Dp >2
            Parameter_uncertainty(1,3) = (Base_parameter_uncertainty + Variable_parameter_uncertainty_inh*m); %[m*1e-1+1e-3 m*2.5e-1+1e-3 m*2.5e-1+1e-3]
            Parameter_std_deviation(1,3) = max(Max_G-Initial_Finhibition,Initial_Finhibition-Min_G);
        end
    end
else
...
Xout = zeros(Dx,Number_sigma,Number_of_observations); % Intialise output states from the point sin the unscented transform matrix
Yout = zeros(Dy,Number_sigma,Number_of_observations); % Intialsie the model output for each set of sigm points matrix
...
Sigma = zeros(Dx,Number_sigma,Number_of_observations);% Intialise the sigma points matrix
...
perc = 0.1; % Percentage change in states expected from one time step to the next

initial_state = initial_mean_calculation_WNM(C,sigmoid_center,perc,dt);
...
State_std_deviation = initial_std_deviation_calculation_WNM(initial_state,C,max_sig,min_sig,lower_percentage,upper_percentage,...
...
X(1:Ds,:,1) = initial_state; % Intialise all parameters for the state estimate
...
Input_std_deviation = Input_mean - Min_frequency; % Define the variance of the input to the model Base 1e-2

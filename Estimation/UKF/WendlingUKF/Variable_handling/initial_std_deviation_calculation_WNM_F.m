% script created by Richard Balson 18/03/2012
...
function [State_std_deviation] = initial_std_deviation_calculation_WNM_F(initial_state,C,max_sig,min_sig,lower_percentage,upper_percentage,...
...
State_std_deviation(:,1) = ones(8,1)*max(max_rangeF)/t_state;
...
State_std_deviation(:,2) = ones(8,1)*max(max_range_state)/t_state;
...
State_std_deviation(:,3) = ones(8,1)*max(max_range_state)/t_state;
...
State_std_deviation(:,4) = ones(Ds,1)*max(max_range_state)/t_state;

% Determine bounds for State 5

low_bound_derivative = 0.1*bottom_voltage_limit/dt;
top_bound_derivative = 0.1*top_voltage_limit/dt;
...
State_std_deviation(:,5) = ones(Ds,1)*max(max_range_state)/t_state;

% Determine bounds for State 6



State_std_deviation(:,6) = 3000;
...
State_std_deviation(:,7) = ones(Ds,1)*max(max_range_state)/t_state;
...
State_std_deviation(:,8) = ones(Ds,1)*max(max_range_state)/t_state;

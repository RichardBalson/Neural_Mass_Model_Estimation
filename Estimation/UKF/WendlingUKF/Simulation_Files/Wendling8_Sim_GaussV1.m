...
std_deviation = (max_frequency-min_frequency)/(2*number_of_sigma_input); % Noise variance
 % Noise std_deviation
...
initial_input = meanf + randn(1)*std_deviation;

normalised_gaussian_input = ones(no_solutions,number_of_changes+1)*initial_input;

normalised_gaussian_inputSDE = ones(no_solutions,number_of_changes+1)*initial_input;% Initialise 8th order matrix
...
for j = 1:(number_of_changes+1)         % Iteration to change parameters in function
    ...
    for k = normalise+1                   % Time Iterations for solver
        ...
        while ((normalised_gaussian_input(k,j) >max_frequency) || (normalised_gaussian_input(k,j) <min_frequency))
           ...
           gauss = randn(1)*std_deviation*stochastic;
            
            normalised_gaussian_input(k,j) = normalised_gaussian_input(k-1,j) + gauss*sqrt(dt);
